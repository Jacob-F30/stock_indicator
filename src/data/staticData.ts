...
export async function loadStaticStock(fallback: Stock): Promise<StaticStockData | null> {
  ...
  return {
    stock: {
      ...
      history: prices,
      operatingMargin: latest ? percent(latest.operating_margin) : fallback.operatingMargin,
      roe: latest ? percent(latest.roe) : fallback.roe,
      roic: latest ? percent(latest.roic) : fallback.roic,
      freeCashFlow: latest ? money(latest.free_cash_flow) : fallback.freeCashFlow,
      cashToMarketCap: latest ? percent(latest.cash_to_market_cap) : fallback.cashToMarketCap,
